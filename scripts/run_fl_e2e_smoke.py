...
CONFIG = "config/fl_e2e_smoke.yaml"
...
def _run_bg(cmd: list[str], env: dict | None = None, log_name: str = "proc") -> subprocess.Popen:
    ...
    if env:
        full_env.update(env)
    log_path = _ROOT / "artifacts" / "runs" / "step06_fl" / "_e2e_logs" / f"{RUN_ID}_{log_name}.log"
...
def main() -> int:
    server_cmd = [PY, "scripts/step05_flower_server.py", "--config", CONFIG, "--run-id", RUN_ID]
    client_cmd = [
        PY,
        "scripts/step06_flower_client.py",
        "--config",
        CONFIG,
        "--server",
        "127.0.0.1:8080",
    ...
    for cid, p in enumerate(clients):
        ...
        log_file = _ROOT / "artifacts" / "runs" / "step06_fl" / "_e2e_logs" / f"{RUN_ID}_client_{cid}.log"
        if log_file.is_file():
    ...
    server_rc = server.wait(timeout=120)
    server_log = _ROOT / "artifacts" / "runs" / "step06_fl" / "_e2e_logs" / f"{RUN_ID}_server.log"
    ...
    post = subprocess.run(
        [
            PY,
            "scripts/step06_fl_post_eval.py",
            "--config",
            CONFIG,
            "--run-id",
            RUN_ID,
    ...
    ckpt = _ROOT / "artifacts" / "checkpoints" / "step06_fedavg_global.pt"
